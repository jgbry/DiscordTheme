...
const Limit = ({ limit, children }: { limit: string | null; children: React.ReactNode }) => (
    <>
        {children}
        <span className={'ml-1 text-[#949ba4] text-[70%] select-none'}>/ {limit || <>&infin;</>}</span>
    </>
);
...
const ServerDetailsBlock = ({ className }: { className?: string }) => {
    ...
    return (
        <div className={classNames('grid grid-cols-2 gap-2 sm:grid-cols-6 sm:gap-2 md:gap-4', className)}>
            ...
            <StatBlock icon={faCloudDownloadAlt} title={'Network (Inbound)'}>
                {status === 'offline' ? <span className={'text-[#949ba4]'}>Offline</span> : bytesToString(stats.rx)}
            </StatBlock>
            <StatBlock icon={faCloudUploadAlt} title={'Network (Outbound)'}>
                {status === 'offline' ? <span className={'text-[#949ba4]'}>Offline</span> : bytesToString(stats.tx)}
            </StatBlock>
        </div>
    );
};
